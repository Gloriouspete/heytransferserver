...
const FileUpload = async (data) => {
  ...
  try {
    const query = `INSERT INTO files (fieldname,fileid,originalname,encoding,mimetype,size,file_data) VALUES(?,?,?,?,?,?,?)`
    executor(query, [data.fieldname, data.fileid, data.originalname, data.encoding, data.mimetype, data.size, data.buffer])
    return true;
  }
  catch (error) {
    console.log(error)
